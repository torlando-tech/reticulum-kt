...
For reading the auto_start preference from the worker context, use `applicationContext.getSharedPreferences()` or read from DataStore directly. Since DataStore is asynchronous and the worker is already a CoroutineWorker, reading via DataStore is acceptable. However, the simpler approach is to use `PreferenceManager.getDefaultSharedPreferences()` or a direct DataStore read. Since the app uses `preferencesDataStore(name = "settings")`, access it via the same DataStore mechanism. The cleanest approach: check if Reticulum.isStarted() as the primary signal. For the auto-restart preference, use a simple SharedPreferences fallback or pass it as input data. Actually, the simplest: use `applicationContext.dataStore` pattern matching what PreferencesManager uses — but since this is in the library module (rns-android), not the app module, use a simpler approach:
- Add a companion object property `autoRestartEnabled: Boolean` that the app sets when starting WorkManager
- Or read from SharedPreferences directly using the DataStore file name ("settings")
- Best approach: Add `inputData` to the WorkRequest with an "auto_restart" key, and read it in doWork() via `inputData.getBoolean("auto_restart", false)`. Update schedule() to accept an autoRestart parameter and pass it via `setInputData()`.
  </action>
  <verify>
