...
files_modified:
  ...
  - rns-core/src/test/resources/lxmf_bridge.py
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "rns-core/src/test/resources/lxmf_bridge.py"
      provides: "Bridge command for Python opportunistic sending"
      contains: "lxmf_send_opportunistic"
  key_links:
    ...
    - from: "PythonToKotlinOpportunisticTest"
      to: "lxmf_bridge.py"
      via: "lxmf_send_opportunistic command"
      pattern: "lxmf_send_opportunistic"
---
...
<task type="auto">
  <name>Task 2: Add lxmf_send_opportunistic to Python bridge</name>
...
<task type="auto">
  <name>Task 3: Create PythonToKotlinOpportunisticTest</name>
  ...
  **Note:** This task depends on Task 2 (the lxmf_send_opportunistic bridge command must exist first).
  ...
  ```
  Tests should run and pass (bridge command now exists from Task 2).
  </verify>
