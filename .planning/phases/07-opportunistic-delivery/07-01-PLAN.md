...
<task type="auto">
  ...
  private suspend fun processOpportunisticDelivery(message: LXMessage) {
      ...
      when {
          ...
          message.deliveryAttempts == MAX_PATHLESS_TRIES + 1 && hasPath -> {
              ...
              // Using Transport.expirePath which removes the path from pathTable
  ...
  7. Use Transport.expirePath(dest.hash) for path removal (equivalent to Python's drop_path)
  </action>
  <verify>
  ```bash
  cd ~/repos/public/reticulum-kt && JAVA_HOME=~/android-studio/jbr ./gradlew :lxmf-core:compileKotlin 2>&1 | tail -20
  ```
  Should compile without errors. Also verify Transport.expirePath is used:
  ```bash
  grep -n "Transport.expirePath\|Transport.requestPath" lxmf-core/src/main/kotlin/network/reticulum/lxmf/LXMRouter.kt
  ```
  </verify>
  <done>processOpportunisticDelivery enhanced with path request logic matching Python: request path after MAX_PATHLESS_TRIES, rediscover at MAX_PATHLESS_TRIES+1 using Transport.expirePath</done>
</task>
...
2. Logic paths verified in code:
   ...
   - Uses Transport.expirePath for path removal
...
- Uses Transport.expirePath for path removal (verified method exists)
