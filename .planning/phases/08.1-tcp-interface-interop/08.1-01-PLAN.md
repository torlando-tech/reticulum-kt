...
must_haves:
  truths:
    - "Diagnostic logging shows socket state, frame boundaries, and packet counts"
...
<task type="auto">
  ...
  <files>rns-test/src/test/kotlin/network/reticulum/integration/TcpPythonInteropTest.kt</files>
  <action>
...
Test structure:
1. **Setup**: Start Python RNS with TCPServerInterface on a random high port using direct subprocess (not bridge)
2. **Test 1 - Connection holds**: Connect, wait 5 seconds, verify still connected
3. **Test 2 - K->P packet**: Send HDLC frame from Kotlin, verify Python received it
4. **Test 3 - P->K packet**: Have Python send HDLC frame, verify Kotlin received it
5. **Test 4 - Bidirectional**: Exchange 10 frames each way, verify counts match
6. **Teardown**: Stop Python subprocess

Implementation notes:
- Use ProcessBuilder to start Python script directly (not bridge_server.py)
- Create minimal Python script inline or as test resource: `python_tcp_test_server.py`
- Python script should:
  - Start RNS TCPServerInterface on specified port
  - Log all received frames to stdout as hex
  - Accept commands on stdin: "SEND <hex>" to send frame, "QUIT" to exit
- Parse Python stdout for "RECEIVED: <hex>" lines
